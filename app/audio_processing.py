...
def process_audio_files(
    ...
    for audio_file in audio_files:
        ...
        recording_details.append(
            prompt_for_recording_details(default_date, default_name, audio_file)
        )
    ...
    for idx, audio_file in enumerate(audio_files):
        ...
        recording_date, recording_name, use_openai = recording_details[idx]
        output_dirs = create_output_dirs(
            base_output_dir, recording_date, recording_name
        )
        ...
        wav_file = os.path.join(
            ...
            os.path.basename(audio_file_no_spaces).replace(".m4a", ".wav"),
        )
        ...
        try:
            ...
            if diarization is not None:
                ...
                if not chunk_files:
                    logging.error(
                        f"No audio chunks found in {output_dirs['temp_chunks']}"
                    )
                    continue
                ...
                for chunk_file in tqdm(
                    chunk_files, desc="Transcribing chunks", leave=False
                ):
                    if use_openai:
                ...
                summary = summarize_transcript(
                    merged_output, summary_prompt_file, openai_api_key
                )
                summary_file = os.path.join(
                    output_dirs["summary"],
                    os.path.basename(wav_file).replace(".wav", "_summary.md"),
                )
                ...
                step_start_time = time.time()
                figure_filename = os.path.basename(wav_file).replace(
                    ".wav", "_waveform.png"
                )
                ...
                logging.warning(
                    f"Skipping transcription and merge for {audio_file_no_spaces} due to missing diarization."
                )

            total_size += os.path.getsize(audio_file_no_spaces)
            total_length += get_audio_length(wav_file)
            os.rename(
                audio_file_no_spaces,
                os.path.join(processed_dir, os.path.basename(audio_file_no_spaces)),
            )

            file_time = time.time() - file_start_time

            logging.info(
                f"Time taken for file {audio_file}: {format_seconds(file_time)}"
            )
            ...
            logging.info(
                f"  - Transcription time: {format_seconds(transcription_time)}"
            )
